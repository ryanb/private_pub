describe("PrivatePub", function () {
  ...
  it("tries to reach faye server before subscribing", function(){
    spyOn($, "ajax")
    var options = {server:"server", channel:"somechannel"}
    pub.sign(options)
    expect($.ajax).toHaveBeenCalled()
    expect($.ajax.mostRecentCall.args[0].url).toBe(options.server)
  })

  describe("when client can connect to faye server", function () {
    beforeEach(function(){
  ...
  describe("when client cannot connect to faye server", function () {
    beforeEach(function(){
      spyOn($, 'ajax').andCallFake(function(options) {
        options.error.call()
      })
    ...
    it("does not add a faye subscription with response handler when signing", function(){
      var faye = {subscribe:jasmine.createSpy('faye')}
      spyOn(pub, 'faye').andCallFake(function (callback) {
        callback(faye);
      });
      var options = {server:"server", channel:"somechannel"};
      ...
      expect(pub.subscriptions.server).not.toEqual("server");
      expect(pub.subscriptions.somechannel).not.toEqual(options);
    })
    ...
    it("logs an error", function() {
      ...
      var options = {server:"server", channel:"somechannel"}
      pub.sign(options)
      expect(console.log).toHaveBeenCalledWith('Cannot connect to pubsub server')
    })
  })
