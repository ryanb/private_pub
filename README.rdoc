...
If you prefer to pass JSON to a custom callback method, you may subscribe to the channel via JavaScript through the global PrivatePub object and specify your callback.
  ...
  <script type="text/javascript">
    PrivatePub.subscribe("/messages/add", function(data) {
...
To publish data as JSON, pass the object you wish to publish to +publish_to+ instead of passing a block. It will have +to_json+ called on it, and the data will be made available to your callback via the first parameter.

  <% publish_to "/messages/new", :message => "Hello, world!", :from => "Ryan Bates" %>

The JSON representation of the object you publish will be available to your JavaScript callback via the first parameter.
