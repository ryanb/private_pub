...
If you prefer to pass JSON to a custom callback method, you may do so using the +:callback+ option to +subscribe_to+.

  <%= subscribe_to "/messages/new", :callback => "process_message" %>

Then, pass the object you wish to render to JSON to +publish_to+ instead of passing a block.
